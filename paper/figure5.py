...
model = deepCR(mask='ACS-WFC-F606W-2-32', inpaint='ACS-WFC-F606W-2-32',device='GPU')
...
for i in range(rows_gal):
    ...
    mask_lacosmic, _ = lac.detect_cosmics(img0 * 100, objlim=4.5, sigclip=20,
                                          sigfrac=0.3, gain=1, readnoise=5, satlevel=np.inf,
                                          sepmed=False, cleantype='medmask', niter=4)
    mask_lacosmic2, _ = lac.detect_cosmics(img0 * 100, objlim=4.5, sigclip=10,
                                           sigfrac=0.3, gain=1, readnoise=5, satlevel=np.inf,
                                           sepmed=False, cleantype='medmask', niter=4)
    ...
    plt.subplot(ax[i, 3])
    plt.imshow(imgg * mask_lacosmic2 * -1, cmap='hot', vmin=vmax, vmax=vmin, alpha=1)
...
ax[0, 2].set_title('true cosmic ray')
ax[0, 1].set_title('deepCR', fontweight="bold")
